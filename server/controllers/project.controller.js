...
export const addUsersToProject = async (req, res) => {
  ...
  try {
    ...
    const updatedProject = await addUsersToProjectService({
      projectId,
      userId,
      users,
    });
...
export const removeUserFromProject = async (req, res) => {
  try {
    const { projectId, userId: removeUserId } = req.body;
    const userId = req.user.id;
...
export const deleteProject = async (req, res) => {
  try {
    ...
    for (const user of deletedProject.users) {
      await redisClient.del(`projects:all:${user}`);
    }
